...
| `1 validation error for ImageConfig / image_size / Extra inputs` (or similar, during `1_plan.py`, the same on every retry) | Your `google-genai` package is older than this pipeline needs — `image_size` didn't exist in `ImageConfig` before version 1.27.0 | `pip install --upgrade google-genai`, then confirm with `pip show google-genai` that it's at least the version `python3 0_check.py` names. This is a code-level mismatch, not a network issue — running it again without upgrading fails the same way every time |
